...
obvious anwer, but you can do better than that. If there was no interest
the debt can be paid off by monthly payments of one-twelfth
...
in monthly installments, because the interest was compounded on the 
balance we didn't pay off each month. So a good upper bound for the
...
Monthly payment upper bound = (Balance x 
                                  (1 + Monthly interest rate)12) / 12.0
...
to find the smallest monthly payment to the cent (no more multiples of
$10) such that we can pay off the debt within a year.Try it out with 
large inputs, and notice how fast it is (try the same large inputs in 
your solution to Problem 2 to compare!). 
...
low = balance / 12.0
high = UpdatedBalanceAfterYear(balance,annualInterestRate,0) / 12.0
fxd = (low+high) / 2.0

endBalance = UpdatedBalanceAfterYear(balance,annualInterestRate,fxd)

while abs(endBalance) >= epsilon: 
    if endBalance < 0.00:
        high = fxd
    elif endBalance > 0.00:
        low = fxd
    else:
        print("wtf")
        break
