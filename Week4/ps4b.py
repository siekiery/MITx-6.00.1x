...
def playGame(wordList):
    ...
    roundCounter = 0
    while True:
        user = input("Enter n to deal a new hand, r to replay the last hand, \
                     or e to end game: ")
        if user == 'n':
            hand = dealHand(HAND_SIZE)
            user = input("Enter u to have yourself play, \
                         c to have the computer play: ")
            while user not in ['c','u']:
            ...
            if user == 'u':
                ...
                raise Exception ("Allowed for invalid user input")
            roundCounter += 1
        elif user == 'r':
            if roundCounter > 0:
                user = input("Enter u to have yourself play, \
